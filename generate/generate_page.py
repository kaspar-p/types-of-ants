...
def main():
    ...
    last_ants_change_git_hash = (
        os.popen('git log --follow -n 1 --pretty=format:"%h" --date=short ants.txt')
        .readlines()
        .pop()
    )

    last_ants_change_git_date = (
        os.popen('git log --follow -n 1 --pretty=format:"%ad" --date=format:"%B %d, %Y" ants.txt')
        .readlines()
        .pop()
    )

    commit_history_length = int(os.popen("git rev-list --count HEAD").readlines().pop())
    ...
    ant_changelist = [apply_ant_rule(ant_line.strip()) for ant_line in ant_changelist]
